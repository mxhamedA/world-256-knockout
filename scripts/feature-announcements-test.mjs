...
assert.match(html, /id="retro2002AnnouncementModal"[\s\S]*World Cup 2002 is here[\s\S]*Play 2002 World Cup/);
assert.doesNotMatch(html, /The UCL simulator is here/);
assert.match(html, /data-open-tournament-theme[^>]*hidden/g);
assert.match(html, /class="tournament-theme-setting"[^>]*hidden/);
...
assert.match(app, /RETRO_2002_ANNOUNCEMENT_KEY = "world-256-announcement-retro-2002-v1"/);
...
assert.match(
  app,
  /!announcementWasSeen\(RETRO_2002_ANNOUNCEMENT_KEY\)[\s\S]*retro2002AnnouncementModal\.showModal\(\)/,
  "The 2002 announcement must appear only when its launch version has not been seen.",
);
assert.match(
  app,
  /retro2002AnnouncementAction\?\.addEventListener\("click"[\s\S]*setRetroWorldCupYear\("2002"\)[\s\S]*startRetroWorldCupButton\?\.click\(\)/,
  "The announcement action must launch the 2002 World Cup mode.",
);
assert.match(app, /newsButton\?\.addEventListener\("click", \(\) => els\.retro2002AnnouncementModal\?\.showModal\(\)\)/, "The shared News button must open the 2002 release announcement.");
assert.match(app, /retroNewsButton\?\.addEventListener\("click", \(\) => els\.newsButton\.click\(\)\)/, "Retro pages must route News through the shared 2002 announcement.");
...
assert.match(css, /\.feature-announcement-2002 \.feature-announcement-accent/);
assert.match(css, /\.feature-announcement-2002-visual/);
