...
for (const year of [256, 1998, 2002, 2006, 2010, 2014, 2016, 2018, 2022, 2026]) {
...
for (const year of [256, 1998, 2002, 2006, 2010, 2016, 2018, 2022, 2026]) {
...
assert.match(css, /achievement-year-tabs[\s\S]{0,240}grid-template-columns:\s*repeat\(10,\s*minmax\(0,\s*1fr\)\)/,
  "All ten tournament achievement tabs must stay in one responsive row.");
assert.match(challenge, /\[256,\s*1998,\s*2002,\s*2006,\s*2010,\s*2014,\s*2016,\s*2018,\s*2022,\s*2026\]/,
...
assert.match(
  challenge,
  /function openRetroAchievementsModal\(year = 2014\)[\s\S]*achievementKey = normalizeAchievementKey\(year\)[\s\S]*achievementKey === "pl"[\s\S]*achievementState\?\.\(\)[\s\S]*trackPremierLeagueSeason\(savedSeason\)[\s\S]*loadAchievements\(year\)/,
